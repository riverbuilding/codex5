...
public class InventoryCatalog {
    ...
    private final ConcurrentMap<String, Integer> unitsSoldByProductId = new ConcurrentHashMap<>();
    ...
    public void addProduct(Product product) {
        ...
        Product previous = products.putIfAbsent(productId, product.copy());
        if (previous != null) {
    ...
    public void recordSale(String productId, int quantity) {
        validateProductId(productId);
        if (quantity <= 0) throw new IllegalArgumentException("recordSale quantity must be positive");
        ...
        products.compute(productId, (id, product) -> {
            ...
            product.decreaseQuantity(quantity);
            unitsSoldByProductId.merge(id, quantity, Integer::sum);
            return product;
        });
    }
    ...
    public List<Product> lowStockProducts(int threshold) {
        ...
        return products.values().stream()
                ...
                .sorted(Comparator.comparingInt(Product::getQuantity).thenComparing(Product::getProductId))
                .map(Product::copy)
                .toList();
    }
    ...
    private Product getInternalProduct(String productId) {
        ...
        if (product == null) throw new NoSuchElementException("Unknown productId: " + productId);
        return product;
    }
